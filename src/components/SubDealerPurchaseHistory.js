...
const SubDealerPurchaseHistory = ({ businessId }) => {
  ...
  const queryDatabase = () => {
    const fetchT = async () => {
      if (!loading) {
        ...
        const resp = await SubDealersAllocationTransactions(
          cFilter,
          pagination
        );
  ...
  const fetchWithPaginate = async () => {
    ...
    if (Object.keys(cleanFil).length > 0) {
      ...
      if (!loading) {
        ...
        const resp = await SubDealersAllocationTransactions(pagination);
        setTransactions(resp);
        setLoading(false);
      }
    }
...
export default SubDealerPurchaseHistory;
