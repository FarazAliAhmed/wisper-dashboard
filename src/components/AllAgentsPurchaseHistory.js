...
import AgentsPurchaseHistoryTable from "./AgentsPurchaseTable";

const AllAgentsPurchaseHistory = ({ businessId }) => {
  ...
  const queryDatabase = () => {
    const fetchT = async () => {
      if (!loading) {
        ...
        const resp = await AllAgentsAllocationTransactions(cFilter, pagination);
  ...
  const fetchWithPaginate = async () => {
    ...
    if (Object.keys(cleanFil).length > 0) {
      ...
      if (!loading) {
        setLoading(true);
        const resp = await AllAgentsAllocationTransactions(pagination);
        setTransactions(resp);
        setLoading(false);
      }
    }
...
export default AllAgentsPurchaseHistory;
