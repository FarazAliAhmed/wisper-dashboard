...
const PricingUser = ({businessId}) => {
  ...
  useEffect(() => {
    const getPlansByUserId = async () => {
      try {
        const response = await axios.get(`${adminUrl}/plans_user/${businessId}`);
  ...
  const createPlanUser = async (plan_id, network, plan_type, price, volume, unit, validity) => {
    try {
      const newPlan = {
        plan_id,
        network,
                        ...
                        {(() => {
                            ...
                            <td>
                                ...
                                <td>
                                    ...
                                    onSubmit={(e) => {
                                        e.preventDefault();
                                        createPlanUser(tdata.dataId, tdata.network, tdata.plan_type, tdata.price, parseFloat(tdata.size), tdata.size.slice(-2), tdata.duration);
                                    }}
                                    >
                                        <button type="submit" 
                                        style={{
