...
import { createSubDealers, register } from "../../services/userService";
...
const AdminSubDealers = ({ businessId }) => {
  const { user } = useUser();
  ...
  useEffect(() => {
    const fetchSubDealers = async () => {
      setLoading(true);
      const resp = await getSubDealers({
        userId: businessId,
      });
  ...
  const handleSearchChange = (e) => {
    ...
    const results = subDealers.filter(
      (business) =>
                      ...
                      <tr key={index} className="border-top">
                        ...
                        <td>
                          <div className="d-flex align-items-center py-2">
                            <Link
                              to={`/subDealer/business/${tdata?.dealer}`}
      ...
      <Modal centered isOpen={success} toggle={() => setSuccess(!success)}>
        <ModalBody>
          <div className="confirm text-center">
            ...
            <p>
              Thank you for adding a sub-dealer to your account. We've sent the
...
export default AdminSubDealers;
